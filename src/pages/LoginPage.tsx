    ...
    return (
        <div style={{ padding: '40px', maxWidth: '1200px', margin: '0 auto', color: 'white' }}>
            ...
            <div style={{
                ...
                <p style={{ margin: '8px 0 0 0' }}>Скидка до 50% на все товары</p>
            </div>

            <div style={{ display: 'flex', gap: '16px', marginTop: '30px', flexWrap: 'wrap' }}>
                {['Электроника', 'Одежда', 'Книга', 'Дом и сад', 'Спорт'].map((category) => (
            ...
            <div style={{
                ...
                {products.map((product) => (
                    <div key={product.id} style={{ 
                        ...
                        <h4 style={{ margin: '10px 0 4px 0', fontWeight: 'bold' }}>{product.name}</h4>                     
                        <p style={{ color: '#b18cff', fontWeight: 'bold' }}>${product.price}</p>
                        <Button variant="primary" size="small" >В коризну</Button>
                    </div>
                ))}
            </div>
        </div>
